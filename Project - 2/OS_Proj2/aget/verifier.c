...
int read_log(char *logfile, struct hist_data *h)
{
//		char *logfile;
		FILE *fp;
		char *user_home;
...
*/
		...
		if(total_bwritten == bwritten) {
				printf("%s is okay. It is safe to resume the download.\n", logfile);
		} else {
				printf("%s is incorrect and may be corrupted. Deleting the log.\n", logfile);
				printf("Total bytes written: %d, Size of content: %d bytes.\n", bwritten, total_bwritten);
				unlink(logfile);
		}
...
int main(int argc, char *argv[]) {
		struct hist_data h;
		if(argc < 2) {
				printf("Usage: ./verifier [log file]\n");
				return 0;
		}
		read_log(argv[1], &h);
		return 0;
